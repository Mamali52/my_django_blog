...
class BlogPostTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.post1 = Post.objects.create(title='Post1', text='my name is javad of post1',
                                        status=Post.STATUS_CHOICES[0][0], author=cls.user, )
        cls.post2 = Post.objects.create(title='Post2', text='my name is javad of post2',
                                        status=Post.STATUS_CHOICES[1][0], author=cls.user, )
    ...
    def test_post_detail(self):
        ...
        self.assertEqual(self.post1.text, 'my name is javad of post1')
    ...
    def test_post_create_view(self):
        response = self.client.post(reverse('post_create'),
                                    {'title': 'Some title', 'text': 'This is some text', 'author': self.user.id,
                                     'status': 'pub'})
    ...
    def test_post_update_view(self):
        response = self.client.post(reverse('post_update', args=[self.post2.id]),
                                    {'title': 'Post2 updated', 'text': 'This text is updated',
                                     'author': self.post2.author.id, 'status': 'pub'})
        self.assertEqual(response.status_code, 302)
